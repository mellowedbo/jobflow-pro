...
export async function PATCH(
  ...
  try {
    ...
    if (body.services !== undefined) {
      ...
      await supabase.from('job_services').delete().eq('job_id', id);
    ...
    if (body.customItems !== undefined) {
      ...
      await supabase.from('job_custom_items').delete().eq('job_id', id);
      if (body.customItems && body.customItems.length > 0) {
        ...
        await supabase.from('job_custom_items').insert(customRows);
      }
    }
    ...
    await logActivity('Job Updated', `Job ${id} updated`, 'job');
...
export async function DELETE(
  ...
  try {
    ...
    await Promise.all([
      supabase.from('job_services').delete().eq('job_id', id),
      supabase.from('job_custom_items').delete().eq('job_id', id),
      supabase.from('job_payments').delete().eq('job_id', id),
      supabase.from('job_internal_costs').delete().eq('job_id', id),
    ]);
    ...
    await logActivity('Job Deleted', `Job ${id} deleted`, 'job');
